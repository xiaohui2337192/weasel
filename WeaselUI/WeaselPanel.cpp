...
void WeaselPanel::Refresh() {
  ...
  // sync scroll mode state from context (set by RimeWithWeasel)
  if (m_style.scroll_mode && m_ctx.cinfo.candies.size() > 0) {
    m_scroll_expanded = m_ctx.cinfo.scroll_expanded;
    m_scroll_row = m_ctx.cinfo.scroll_row;
    m_scroll_col = m_ctx.cinfo.scroll_col;
  }
  ...
  // reset the sticky state and scroll mode state
  if (m_lastCandidateCount > 0 && m_candidateCount == 0) {
    m_sticky = false;
    m_scroll_expanded = false;
    m_scroll_row = 0;
    m_scroll_col = 0;
  }
...
bool WeaselPanel::_DrawCandidates(CDCHandle& dc, bool back) {
  ...
  // scroll mode: draw row highlight background
  bool is_scroll_row_hilite = m_style.scroll_mode && m_scroll_expanded && 
      COLORNOTTRANSPARENT(m_style.scroll_row_back_color);
  int scroll_row_start = m_scroll_row * 5;
  int scroll_row_end = min(scroll_row_start + 5, (int)m_candidateCount);

  CRect rect;
  // draw back color and shadow color, with gdi+
  if (back) {
    // scroll mode: draw row highlight background first
    if (is_scroll_row_hilite && scroll_row_end > scroll_row_start) {
      CRect rowRect;
      rowRect.left = INT_MAX;
      rowRect.top = INT_MAX;
      rowRect.right = 0;
      rowRect.bottom = 0;
      for (auto i = scroll_row_start; i < scroll_row_end && i < MAX_CANDIDATES_COUNT; ++i) {
        CRect candRect = m_layout->GetCandidateRect(i);
        if (m_istorepos)
          candRect.OffsetRect(0, m_offsetys[i]);
        rowRect.left = min(rowRect.left, candRect.left);
        rowRect.top = min(rowRect.top, candRect.top);
        rowRect.right = max(rowRect.right, candRect.right);
        rowRect.bottom = max(rowRect.bottom, candRect.bottom);
      }
      if (rowRect.right > rowRect.left) {
        rowRect.InflateRect(DPI_SCALE(m_style.hilite_padding_x),
                           DPI_SCALE(m_style.hilite_padding_y));
        IsToRoundStruct rd;
        _HighlightText(dc, rowRect, m_style.scroll_row_back_color, 0x00000000,
    ...
    if ((COLORNOTTRANSPARENT(m_style.candidate_back_color) ||
      ...
      for (auto i = 0; i < m_candidateCount && i < MAX_CANDIDATES_COUNT; ++i) {
        if (i == m_ctx.cinfo.highlighted || i == m_hoverIndex)
          continue;
        // skip scroll row items (already drawn with row background)
        if (is_scroll_row_hilite && i >= scroll_row_start && i < scroll_row_end)
          continue;
        rect = m_layout->GetCandidateRect((int)i);
        IsToRoundStruct rd = m_layout->GetRoundInfo(i);
        if (m_istorepos) {
    ...
    // draw highlighted background and shadow
    {
      ...
      rect.InflateRect(DPI_SCALE(m_style.hilite_padding_x),
                       DPI_SCALE(m_style.hilite_padding_y));
      // scroll mode: use scroll_row_hilited_candidate_back_color for highlighted item in scroll row
      int hilite_color = m_style.hilited_candidate_back_color;
      if (is_scroll_row_hilite && COLORNOTTRANSPARENT(m_style.scroll_row_hilited_candidate_back_color))
        hilite_color = m_style.scroll_row_hilited_candidate_back_color;
      _HighlightText(dc, rect, hilite_color,
