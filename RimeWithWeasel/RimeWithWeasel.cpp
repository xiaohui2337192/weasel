...
BOOL RimeWithWeaselHandler::ProcessKeyEvent(KeyEvent keyEvent,
  ...
  if (session_status.style.scroll_mode && !(keyEvent.mask & ibus::Modifier::RELEASE_MASK)) {
    RIME_STRUCT(RimeContext, ctx);
    bool has_context = rime_api->get_context(session_id, &ctx);
    int total_candidates = has_context ? ctx.menu.num_candidates : 0;
    rime_api->free_context(&ctx);
    
    if (total_candidates > 0) {
      bool key_handled = false;
      int candidates_per_row = 5;
      int initial_count = session_status.style.scroll_mode_initial_lines * candidates_per_row;
      int max_rows = session_status.style.scroll_mode_max_lines;
      
      // handle arrow keys
      if (keyEvent.keycode == ibus::Keycode::Down) {
        if (!session_status.scroll_expanded) {
          // expand and jump to row 1 (second row)
          if (total_candidates > initial_count) {
            session_status.scroll_expanded = true;
            session_status.scroll_row = 1;
            session_status.scroll_col = 0;
            key_handled = true;
          }
        } else {
          int max_row = min((total_candidates - 1) / candidates_per_row, max_rows - 1);
          if (session_status.scroll_row < max_row) {
            session_status.scroll_row++;
            int max_col = min(candidates_per_row - 1, total_candidates - 1 - session_status.scroll_row * candidates_per_row);
            if (session_status.scroll_col > max_col)
              session_status.scroll_col = max_col;
            key_handled = true;
          }
        }
      } else if (keyEvent.keycode == ibus::Keycode::Up) {
        if (session_status.scroll_expanded) {
          if (session_status.scroll_row == 1) {
            // collapse back to first row
            session_status.scroll_expanded = false;
            session_status.scroll_row = 0;
            session_status.scroll_col = 0;
            key_handled = true;
          } else if (session_status.scroll_row > 0) {
            session_status.scroll_row--;
            session_status.scroll_col = 0;
            key_handled = true;
          }
        }
      } else if (keyEvent.keycode == ibus::Keycode::Right) {
        if (session_status.scroll_expanded) {
          int max_col = min(candidates_per_row - 1, total_candidates - 1 - session_status.scroll_row * candidates_per_row);
          if (session_status.scroll_col < max_col) {
            session_status.scroll_col++;
            key_handled = true;
          }
        }
      } else if (keyEvent.keycode == ibus::Keycode::Left) {
        if (session_status.scroll_expanded && session_status.scroll_col > 0) {
          session_status.scroll_col--;
          key_handled = true;
        }
      }
      
      // handle digit keys 1-5 for quick select in scroll mode
      if (session_status.scroll_expanded && keyEvent.keycode >= '1' && keyEvent.keycode <= '5') {
        int col = keyEvent.keycode - '1';
        int idx = session_status.scroll_row * candidates_per_row + col;
        if (idx < total_candidates) {
          rime_api->select_candidate_on_current_page(session_id, idx);
          _Respond(ipc_id, eat);
          _UpdateUI(ipc_id);
          m_active_session = ipc_id;
          return TRUE;
      ...
      if (key_handled) {
        // update UI without processing key through rime
        _Respond(ipc_id, eat);
        _UpdateUI(ipc_id);
        m_active_session = ipc_id;
        return TRUE;
...
void RimeWithWeaselHandler::_UpdateUI(WeaselSessionId ipc_id) {
  ...
  // set scroll mode state in context
  if (session_status.style.scroll_mode && !weasel_context.cinfo.empty()) {
    weasel_context.cinfo.scroll_expanded = session_status.scroll_expanded;
    weasel_context.cinfo.scroll_row = session_status.scroll_row;
    weasel_context.cinfo.scroll_col = session_status.scroll_col;
    // update highlighted to current scroll position
    int scroll_highlighted = session_status.scroll_row * 5 + session_status.scroll_col;
    if (scroll_highlighted < (int)weasel_context.cinfo.candies.size())
      weasel_context.cinfo.highlighted = scroll_highlighted;
    // when not expanded, limit displayed candidates
    if (!session_status.scroll_expanded) {
      int initial_count = session_status.style.scroll_mode_initial_lines * 5;
      if ((int)weasel_context.cinfo.candies.size() > initial_count) {
        weasel_context.cinfo.candies.resize(initial_count);
        weasel_context.cinfo.comments.resize(initial_count);
        weasel_context.cinfo.labels.resize(initial_count);
      }
    }
  }
...
static void _UpdateUIStyle(RimeConfig* config, UI* ui, bool initialize) {
  ...
  // get scroll mode settings
  _RimeGetBool(config, "style/scroll_mode", initialize, style.scroll_mode, true,
               false);
  _RimeGetIntStr(config, "style/scroll_mode_initial_lines",
                 style.scroll_mode_initial_lines);
  if (style.scroll_mode_initial_lines <= 0)
    style.scroll_mode_initial_lines = 1;
  _RimeGetIntStr(config, "style/scroll_mode_max_lines",
                 style.scroll_mode_max_lines);
  if (style.scroll_mode_max_lines < style.scroll_mode_initial_lines)
    style.scroll_mode_max_lines = style.scroll_mode_initial_lines;
  _RimeGetIntStr(config, "style/scroll_row_back_color", style.scroll_row_back_color);
  _RimeGetIntStr(config, "style/scroll_row_hilited_candidate_back_color",
                 style.scroll_row_hilited_candidate_back_color);
  // get color scheme
  const int BUF_SIZE = 255;
  char buffer[BUF_SIZE + 1] = {0};
